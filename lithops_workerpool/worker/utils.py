...
from .globals import worker_cache
...
def get_function_and_modules(job, internal_storage):
    ...
    if customized_runtime:
        ...
        with open(func_path, "rb") as f:
            func_obj = f.read()
        function = pickle.loads(func_obj)
    else:
        if job.func_hash not in worker_cache:
            logger.debug('Getting function and module data from storage')
            func_obj = internal_storage.get_func(job.func_key)
            loaded_func_all = pickle.loads(func_obj)
            function = loaded_func_all['func']
            worker_cache[job.func_hash] = function
            ...
            logger.debug('Getting function and module data from cache')
            function = worker_cache[job.func_hash]
